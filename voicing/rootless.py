...
def root_omission_condition_3(chord: ChordEvent, degrees_without_root: list[Degree],
                               gen_dir: str = None) -> Optional[tuple]:
    ...
    rootless_pcs = frozenset((chord.root_interval + d.semitone) % 12 for d in degrees_without_root)
    return rootless_collision(rootless_pcs, chord.root_interval % 12, chord.chord_type(), **kwargs)
...
def resolve_root_omission(chord: ChordEvent, degrees: list[Degree],
                           ...
                           gen_dir: str = None) -> tuple[bool, dict]:
    ...
    collision = root_omission_condition_3(chord, degrees_without_root, gen_dir)
    ...
    if not has_upper_ext:
        return False, {}
    if rng.random() < 0.5:
        return False, {}  # Branch A: keep the root.
    flags = {"branch_b": True, "force_dct_top": True}
