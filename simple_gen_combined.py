"""
dual_generator.py — Single-voice chord generator (Dual Output)
===================================================
Generates chords and outputs TWO JFugue Staccato strings per song:
  1. Fixed Octave version (PadComposerFixed)
  2. Voice-Leading Anchor version (PadComposerVoiceLeading)
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# PadComposer Implementations (Dual Output)
# ─────────────────────────────────────────────────────────────────────────────

class PadComposerFixed:
    _DEFAULT_INST = 0

    def __init__(self, chords: list[dict], tonic_pc: int = 0, inst: int = _DEFAULT_INST):
        self.chords    = chords
    ...
    def _measure_pad(self, chord: dict) -> str:
        ...
        # anchor the tonic to MIDI 62 (D4), then voice root relative to that
        TARGET_CENTER = 62
...
class PadComposerVoiceLeading:
    """
    Implementation 2: Voice-Leading Anchor.
    Features smooth voice-leading with an anti-drift anchor.
    """
    _DEFAULT_INST = 0

    def __init__(self, chords: list[dict], inst: int = _DEFAULT_INST):
        self.chords = chords
        self.inst   = inst
        self.prev_upper_avg = 64.0 
    ...
    def _measure_pad(self, chord: dict) -> str:
        ...
        base_notes = [root_pc + s for s in sorted(set(triad_semis))]

        seventh_semi = EXT_TO_SEMI.get(chord.get("seventh", "N"))
        if seventh_semi is not None:
            seventh_base = root_pc + seventh_semi
            fifth_base   = root_pc + triad_semis[-1]
            if seventh_base <= fifth_base:
                seventh_base += 12
            base_notes.append(seventh_base)

        for key in ("ninth", "eleventh", "thirteenth"):
            semi = EXT_TO_SEMI.get(chord.get(key, "N"))
            if semi is not None:
                base_notes.append(root_pc + semi + 12)

        core_notes = [root_pc + s for s in sorted(set(triad_semis))]
        if seventh_semi is not None:
            core_notes.append(root_pc + seventh_semi)
        core_avg = sum(core_notes) / len(core_notes)

        anchor = 64.0
        target_avg = (self.prev_upper_avg * 0.7) + (anchor * 0.3)
        octaves_to_shift = round((target_avg - core_avg) / 12)
        midi_notes = [n + (octaves_to_shift * 12) for n in base_notes]
        
        self.prev_upper_avg = sum(midi_notes) / len(midi_notes)
        midi_notes = [m for m in midi_notes if m % 12 != bass_pc]
...
def _parse_args():
    p = argparse.ArgumentParser(description="Simple dual chord-sequence generator")
    ...
    p.add_argument("--songs",       type=int, default=5,  help="Number of songs to generate")
    p.add_argument("--temperature", type=float, default=1.0)
    p.add_argument("--bpm",         type=int, default=120)
    p.add_argument("--seed",        type=int, default=None)
    p.add_argument("--dist-dir",    default=_DEFAULT_DIST_DIR)
    p.add_argument("--out-dir",     default="gen/labels", help="Output directory for .lab files")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    print(f"Starting dual batch generation of {args.songs} songs...")
    print(f"BPM: {args.bpm} | Genre: {args.genre.upper()}")

    with open(OUTPUT_TXT_FILE, "w") as f:
        for i in range(args.songs):
            print(f"  -> Generating track {i} (Dual Output)...")

            gen = ChordGenerator(
                genre       = "jazz", # args.genre,
                tonic_pc    = random.randint(0, 11), # tonic,
                num_chords  = args.num,
            ...
            jfugue_fixed = PadComposerFixed(chords, tonic_pc=gen.tonic_pc).render_to_jfugue(bpm=args.bpm)
            jfugue_vl    = PadComposerVoiceLeading(chords).render_to_jfugue(bpm=args.bpm)

            # Write Fixed Version
            f.write(f"START_SONG_{i}_FIXED\n")
            f.write(jfugue_fixed + "\n")
            f.write("END_SONG\n")
            
            # Write Voice-Leading Version
            f.write(f"START_SONG_{i}_VOICE_LEADING\n")
            f.write(jfugue_vl + "\n")
            f.write("END_SONG\n")

    print(f"\nSuccess! Wrote dual rendering instructions to {OUTPUT_TXT_FILE}")
