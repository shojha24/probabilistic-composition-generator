...
`label x genre x melody instrument x register x rhythm/behavior`. Count
instrument, register, density, rhythm, and behavior realizations, and do not
treat copies of one source progression as independent chord evidence. The 70%
melody gate should be audited after realization; its aggregate probability is
not a substitute for inspecting realized corpus coverage.
