...
- **Status:** research supplement and implementation rationale
- **Companion spec:** `specs/14-melody-generation.md`
...
6. **Keep chord labels authoritative and melody output explicit.** The
   `ChordEvent` sequence must not be changed, rejected, or locally relabeled
   because of melody generation. Melody should be a naturalistic full-mix
   augmentation whose instrument, rhythm, register, and melodic behavior are
   recorded. The synchronized multitrack output lets downstream consumers
   select the parts they need without requiring matched renders.
7. **Treat audio recognition as a later consumer evaluation.** The first
   implementation should focus on symbolic correctness and naturalistic
   full-mix generation. Chord recognition, melody extraction, masking, and
   source separation can be evaluated later as separate tasks.
...
its effect on chord-recognition audio. That gap should be handled first with
symbolic validation, naturalistic coverage reporting, and explicit multitrack
provenance. Model-level comparisons can follow once the corpus is available.
...
## Long-tail objective: naturalistic full-mix melody

The primary output of this repository is a labeled chord progression for
long-tail chord-recognition data. Melody is a **naturalistic conditional
...
ChordGenerator output and rare-feature quotas
    ...
    -> chord-conditioned naturalistic melody
    -> full mix plus synchronized multitrack scores
    -> manifest metadata
```

For ACR, the source chord event remains higher in the hierarchy than every
rendered or generated part:

```text
ChordEvent label
    > VoicedChord realization
    > V0/V1/V9 accompaniment rendering
    > optional V2 melody augmentation
```

The melody specification is intended to resolve most previously unaddressed
implementation details while preserving that hierarchy. It may define how
melody candidates are selected, sequenced, rendered, and recorded, but it
must not alter the source label or use a melody note as a replacement for
missing chord evidence. The full mix may change the difficulty of a downstream
ACR task, but it must never change the answer that the sample is labeled with.
...
The mixed score and each role sidecar must therefore share the same source
label hash, chord-event sequence, event timing, and output ordinal. Melody
metadata may differ in density, register, instrument, rhythm, or behavior, but
the ACR target must not.

### Full-mix data and selectable parts

There are two separate failure modes:

1. **Label drift:** the symbolic target changes. This is prohibited by the
   source-event invariant.
2. **Downstream shortcut learning:** a model may use a melody instrument,
   register, density, or recurring pitch instead of chord evidence even when
   every melody note is musically valid.

The first implementation addresses the first failure directly and exposes the
parts needed to investigate the second later. Spec 13.5 provides the mixed
score and synchronized chords, bass, percussion, and melody sidecars. A
consumer can therefore train or evaluate on the full mix or select any
subset without requiring the generator to create a separate accompaniment
control or matched melody render.

Naturalistic generation should still vary instruments, rhythms, registers,
density, contour, and harmonic behavior across the corpus. Record those
attributes so a later model evaluation can identify their contribution, but
do not make a full cross-condition ablation a prerequisite for implementation.

### Why more chord events alone are insufficient

More than 250,000 chord events may be useful for rare-label coverage, but raw
volume addresses sampling variance rather than confounding bias. If every
example of a label also tends to use the same melody instrument, register,
density, or exposed pitch, a larger corpus gives the model more evidence for
the same shortcut.

The relevant unit for planning is independent coverage of
`label x genre x melody instrument x register x rhythm/behavior`. Count instrument, register, density, rhythm, and behavior realizations, and do
not treat copies of one source progression as independent chord evidence. The
70% melody quota should be audited after realization; its exact song-level
target is not a substitute for inspecting realized corpus coverage.

Additional generation is justified when it supplies new chord progressions,
voicings, event timings, accompaniment instruments, and naturalistic melody
realizations for under-covered areas. It is not justified merely because the
global event counter is below a larger target. A later model study can use
the exported sidecars to isolate or remix parts without regenerating melody
variants.
...
- the naturalistic profile, behavior weights, and its random seed.
...
`A, C#, E, G, Bb`. A naturalistic melody can use `C#` or `Bb` on selected
accents and resolve them toward neighboring material without declaring a
D-major key for that event. The altered chord remains the label-bearing
object. The realized role and resolution are recorded in the V2 event map,
while consumers can decide whether to use V2 in a later mix.
...
**Implication:** preserve the generated melody and accompaniment as
synchronized symbolic tracks. If a later study selects or separates parts,
record the selected tracks and report separation quality independently from
recognition quality.
...
**Implication:** keep source separation outside the initial melody
implementation. Do not use separated audio to overwrite symbolic labels or to
claim that masking was solved.
...
**Implication:** this supports a possible later audio study. It does not
require source separation, unseparated controls, or oracle-stem experiments
for the initial naturalistic corpus.

## Recommended architecture for naturalistic full-mix melody

The melody implementation should be downstream of chord generation and should
produce a reproducible, inspectable full-mix realization of each completed
chord sequence. It is not a second harmonic generator. Chord-event selection,
rare-feature quotas, transition support, and chord labels remain owned by the
existing pipeline.
...
stylistically stable extension according to the selected naturalistic profile.
For rare or altered events, the DCT and active extension may receive
profile-weighted preference without creating a separate matched condition.
Weak positions can carry tension when the candidate resolves over a short
horizon.
...
The practical symbolic MVP should use a seeded, bounded chord-centered random
walk. It should choose among active chord-degree candidates resolved near the
previous melody pitch, with metric/profile weights, distance and maximum-leap
transition weights, and explicit hold/rest candidates. It should not sample
arbitrary MIDI pitches and should not propose unsupported non-chord tones in
the naturalistic policy.

A later beam search, Viterbi decoder, or other whole-phrase model can reuse
the same candidate features. In either case, a candidate sequence score should
combine:
...
- manifest records for melody configuration, seed, profile behavior, and
...
Validate the naturalistic generator without requiring a condition or decoder
ablation. Measure:

- duration-grid coverage and timing validity;
- note/rest and rhythm density;
- instrument, register, and range coverage;
...
- cadence completion;
- no-chord violations;
- source-degree versus realized-voicing mismatch; and
- fallback rate.

Report these metrics by genre, melody profile, and chord type, especially
altered and extension-rich events. A high global chord-tone rate can hide a
generator that fails specifically on chromatic chords, so behavior counts and
resolution diagnostics remain required.

### Full-mix and multitrack inspection

The primary audio artifact is the naturalistic full mix. Validate that the
mixed score and synchronized V0/V1/V2/V9 sidecars have identical song
ordinals, source hashes, and timelines. Inspect representative variation in
melody instrument, rhythm, register, density, contour, rests, holds, and
harmonic behavior.

The sidecars are the available part-selection mechanism. A later consumer may
use the full mix, remove V2, isolate V2, or remix any combination without
requiring matched melody variants to be generated. Any downstream study should
record its selected tracks and mix settings.

### Deferred model comparison

Model-level evaluation is deferred until the naturalistic full-mix corpus is
available. The planned comparison is:

1. a model trained on preexisting Billboard data;
2. Billboard data plus the chord corpus; and
3. Billboard data plus the chord corpus with naturalistic melody included.

This is a corpus contribution comparison, not a required cross-condition
ablation of melody policies. The multitrack manifest records which parts are
included in each later training or evaluation input.

## Realism versus full-mix utility
...
to recognize, especially when it exposes a DCT or altered extension. The
naturalistic generator should avoid making one fixed pitch, instrument, or
rhythm the only representation of a label by sampling across its configured
profiles and behavior weights.
...
an inspectable melodic behavior with a target and resolution, not treated as
one binary quality judgment.

The most useful first target is a **naturalistic full-mix envelope**:

1. generate chord-aware melodies with bounded range, density, intervals, and
   short-horizon resolution;
2. vary instruments, rhythms, registers, density, contour, and harmonic
   behavior through the naturalistic profile configuration;
3. preserve the exact chord event, label, and source timeline;
4. validate the full mix and synchronized V0/V1/V2/V9 sidecars; and
...
If a later recognizer performs differently on a dense or chromatic naturalistic
mix, that is not necessarily a failed generator. It is a downstream result to
record alongside the selected track mix and melody metadata. The symbolic
failure is only a problem if the melody violates its source-event, timing, or
resolution contract.
...
## Research-to-spec decisions

This section turns the findings above into implementation guidance for this
repository. A research finding supports the direction of a feature; it does
not automatically supply a universal probability, score weight, phrase
length, or instrument range. Items marked as **repository defaults** are
bounded engineering choices for a first implementation and must remain
configurable and auditable.

### Evidence-to-decision matrix

| Concern | Research-supported finding | Repository decision | Status |
|---|---|---|---|
| Chord tones versus non-chord tones | Metric position, duration, arrival interval, departure interval, and near-term context are useful predictors of CT/NCT treatment. | Store these features on candidate/diagnostic records and use them in the sequence score. Do not use a fixed CT/NCT percentage. | Evidence-backed |
| Tension and resolution | Harmonic function, sensory dissonance, and horizontal motion contribute separately to perceived tension. | Treat chromaticity and NCT status as soft costs. Reward a plausible short-horizon resolution; do not reject a note solely because it is non-diatonic. | Evidence-backed |
| Global and local tonality | Key perception is contextual, and key changes need persistence or phrase-level evidence. | Use global tonic/mode as a weak prior. Defer local-key inference to a bar/phrase model with a transition cost; never reset the key at every chord. | Evidence-backed |
| Long-range structure | Hierarchical models improve long-sequence structure by separating coarse subsequences from local note generation. | Generate a phrase/rhythm plan before pitch realization. Use a bounded chord-centered random walk for the MVP, with a lookahead/beam decoder as a later option instead of independent slot draws. | Direction supported; exact sizes are defaults |
| Instrument range | General MIDI standardizes program identity, not realistic acoustic range or timbre. | Store hard range, soft tessitura, and preferred center per melody instrument. Validate against the target soundfont separately. | Evidence-backed |
| Rhythm and density | Published systems demonstrate useful phrase hierarchies, but do not establish universal note-density or phrase-length distributions. | Expose density and phrase parameters, measure them by genre/profile, and calibrate from a melody reference corpus when one is available. | Evidence-backed negative |
| Masking | Interference depends on frequency, spectrum, level, duration, register, and timbre; high register alone is not a guarantee of salience. | Keep register, density, velocity, and timbre as independent experimental factors. Keep symbolic masking costs optional and disabled in the MVP. | Evidence-backed |
| Per-timepoint label safety | Symbolic source labels and audio recognizer decisions are different objects; added melody can change the latter without changing the former. | Use one `naturalistic` label-safety policy. Enforce source-event/NCT rules and do not infer audio-level ACR stability from them. | Evidence-backed distinction |
| Audio evaluation | Melody extraction and source separation are distinct inference problems with their own failure modes. | Preserve V2 and the synchronized multitrack roles; defer audio-model comparisons until the full-mix corpus is available. | Evidence-backed |

The CT/NCT feature recommendation is based most directly on Hu and Arthur's
statistical melody-reduction model. The sequence and hierarchy recommendation
is consistent with MusicVAE, hierarchical melody/harmony systems, and
probabilistic harmonization work. The tonal-context recommendation follows
Krumhansl and Kessler, Temperley, and contextual chord-estimation research.
The masking boundary follows psychoacoustic masking work and melody-extraction
surveys. These sources justify the feature families and experimental controls,
not a claim that the generated output is musically realistic without corpus or
listening evaluation.

### Initial symbolic implementation defaults

The following defaults make a first implementation deterministic and
testable while keeping unsupported assumptions visible:

```text
time_grid                  = 1 sixteenth note
meter                       = 4/4 until Song exposes meter
subphrase_length            = 2 bars
phrase_context_window       = 8 bars
cadence_context_window      = 2 bars
nct_resolution_horizon      = next 2 melody attacks
beam_width                  = 32 states
candidate_limit_per_onset   = 24 candidates after octave selection
decoder                     = chord_centered_random_walk
local_key_inference         = disabled
label_safety_policy         = naturalistic
symbolic_masking_cost       = disabled
```

These are repository engineering defaults, not measurements from a universal
melody corpus. The 2-bar subphrase and longer phrase context are an
implementation-scale interpretation of hierarchical-generation evidence, not
a requirement that every song use a 16-bar form. The implementation may use a
different value only through a named profile/configuration, and the effective
values must be written to the manifest.

The first decoder should apply the following order:

1. **Hard validity:** timeline, positive sixteenth-unit duration, MIDI
   0--127, profile hard range, valid source mapping, monophonic overlap, and
   default no-chord silence.
2. **High-priority musical terms:** active chord-degree compatibility on strong
   positions, profile-weighted extension preference, and short-horizon
   resolution for approach/passing/neighbor/enclosure candidates.
3. **Continuity terms:** interval size, contour direction, held-note
   continuity, phrase repetition, cadence, and soft tessitura distance.
4. **Lower-priority context terms:** global-scale fit, realized-voicing
   overlap, density preference, and optional masking proxy.

This ordering prevents a diatonic but label-insensitive note from defeating an
active altered degree merely because it belongs to the global scale. It also
keeps a soft musical preference from overriding a serialization or
label-preservation invariant.

### Per-timepoint ACR safety interpretation

For a source event interval `[start, end)`, the ACR target is the complete
`ChordEvent` tuple attached to that interval. Melody generation does not
recompute that target from the union of V0, V1, V2, and V9 pitches. The
validator should compare the pre-melody and post-melody source timelines
directly, not infer label equality from an audio recognizer.

The `naturalistic` policy is the default symbolic safety envelope:

- strong-position onsets are active chord degrees or stable extensions, or
  explicitly resolved NCTs;
- a non-chord tone carries a role and target and resolves within the configured
  lookahead;
- a held pitch that becomes non-chordal at a playable event boundary is
  represented as a suspension/hold and is resolved or clipped explicitly;
- no-chord intervals remain rests; and
- the melody is serialized on V2 only.

The naturalistic policy can allow dense, chromatic, or same-register material
when it is musically supported, but it cannot modify the source label,
source-event duration, or source identity. This distinction prevents a
successful symbolic preservation check from being misreported as proof of
audio-level recognizer stability.

### Candidate and NCT metadata required for scoring

Every non-rest candidate should retain enough information to explain its
selection:
...
duration_sixteenths
arrival_interval
departure_interval         # populated when a successor is evaluated
resolution_target
role
pitch_source
```

The `departure_interval` and `resolution_target` are sequence features, so
they may be unknown during initial enumeration and must be filled or scored
when a successor is considered. A candidate that is an approach, passing,
neighbor, enclosure, suspension, or appoggiatura is not valid merely because
its pitch class is close to a target; the role must include an inspectable
target and the selected sequence must satisfy the configured resolution
horizon. A strong-beat NCT is allowed when its naturalistic profile and
short-horizon resolution score justify it.

### Range and timbre contract

The melody profile must distinguish:

```text
hard_min, hard_max             # never emit outside
tessitura_min, tessitura_max   # soft comfort region
preferred_center               # soft register target
```

Approximate reference anchors from orchestration material are useful for
initial profiles, but are not a substitute for soundfont validation:

| Instrument | Approximate reference MIDI range |
|---|---:|
| Violin | 55--105 |
| Flute | 60--98 |
| Clarinet | 52--96 written/reference range |
| Trumpet | 54--86 written/reference range |

The range anchors are adapted from Frans Absil's orchestration range chart
([reference PDF](https://www.fransabsil.nl/archpdf/musical_instrument_range.pdf)).
The General MIDI specification is useful for program identity but explicitly
does not define the acoustic behavior of a patch
([GM Level 1 overview](https://midi.org/general-midi-level-1)).

Transposing-instrument conventions and the actual target soundfont must be
recorded. A GM program number alone must never be used to infer a playable
range. The validator should report hard-range violations separately from
soft-tessitura excursions.

### Required calibration before claiming realism

This repository currently has chord labels and accompaniment metadata, but no
authoritative reference melody corpus. Therefore the first implementation must
not present its initial CT/NCT ratio, rhythm distribution, phrase length, or
instrument tessitura as learned facts. It should:

1. emit naturalistic configuration values and effective values in the manifest;
2. report CT/NCT, role, density, duration, interval, range, and resolution
   distributions by genre, profile, instrument, rhythm, register, and behavior;
3. compare those distributions with a clearly identified reference corpus
   before changing defaults;
4. keep externally calibrated priors separate from the generated chord labels;
   and
5. retain the synchronized multitrack export so later consumers can select
   parts without regenerating the melody.

When no reference corpus is available, acceptance should focus on invariants,
reproducibility, role/provenance correctness, naturalistic coverage, and
multitrack pairing rather than an unsupported claim of human-likeness.

### Explicitly unsupported universal claims

The cited work does **not** justify hard-coding any of the following:

- one CT/NCT ratio for jazz, pop/rock, and all chord types;
- one note density or phrase length for every instrument or genre;
- one universal maximum leap or octave correction rule;
- playable ranges inferred only from GM program numbers;
- a rule that every strong beat must be a chord tone;
- a rule that every non-chord tone must be rejected;
- a rule that a high-register melody is always unmasked;
- a per-chord key or scale reset; or
- a claim that source separation improves chord recognition in every mix.

## Bottom line

The strongest implementation path is a symbolic, chord-event-conditioned
melody augmenter with a weak global tonal context, a chord-centered random-walk
MVP, optional phrase-level local-key inference, weighted chromatic
tension/resolution, and varied naturalistic profile behavior. The predecessor's
rhythmic and phrase mechanics can accelerate the first prototype, but its
final-chord anchoring should be replaced rather than copied.

For the initial corpus, render naturalistic full mixes and synchronized V2
sidecars alongside V0/V1/V9. This preserves the project's symbolic ground
truth while allowing later users or model studies to select accompaniment,
melody, or mixed parts without requiring matched variants or a full ablation
