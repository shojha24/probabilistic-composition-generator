...
melody candidates are selected, sequenced, exposed, withheld, rendered, and
evaluated, but it must not alter the source label or use a melody note as a
replacement for missing chord evidence. A melody condition can change the
difficulty of ACR, and that change is an intended measurable experiment; it
must never change the answer that the sample is labeled with.
...
All matched melody variants derived from one progression must therefore share
the same source label hash, chord-event sequence, event timing, dataset split,
and source identity. Variant metadata may differ in melody condition,
density, register, instrument, or exposure, but the ACR target must not.

### False-signal risk in ACR training
...
2. **Shortcut learning:** the target stays fixed, but melody properties become
   correlated with the target, so a model learns the melody instrument,
   register, density, or a recurring pitch instead of the chord evidence.

The second failure can occur even when every melody note is musically valid.
For example, always exposing a rare extension with the same lead instrument
can make that instrument/pitch combination a label shortcut. A transparent
symbolic policy reduces unsupported clashes but cannot prove that a rendered
mix is statistically label-neutral.

Use two explicitly separated dataset roles:

| Dataset role | Contents | Interpretation |
|---|---|---|
| `core_acr` | accompaniment-only controls plus balanced `transparent` melody variants | Primary chord-learning distribution |
| `acr_robustness` | `stress`, dense, chromatic, and intentionally exposed variants | Controlled nuisance and failure analysis |

Within `core_acr`, balance melody presence and melody attributes within each
label, genre, and rare-feature stratum. The same source progression should
have a no-melody control and, where practical, multiple melody variants. Use
source-level splits before variant generation. Keep exposed DCT/extension
variants out of the sole primary training distribution; otherwise the model
can learn that exposure pattern rather than the accompaniment's chord
evidence.

The minimum shortcut checks are:

- train and test on accompaniment-only;
- train on accompaniment-only and test on transparent melody-on variants;
- train on balanced mixed conditions and test on a held-out melody condition;
- compare confusion by chord label and melody condition; and
- probe whether melody-only features predict labels above the intended
  baseline.

If melody-on accuracy changes materially while the accompaniment is identical,
that is evidence about nuisance sensitivity or shortcut structure, not proof
that the underlying chord label changed. Preserve the result as a labeled
experiment rather than silently folding it into the core ACR score.
...
The relevant unit for planning is independent coverage of each
`label x genre x accompaniment condition x melody condition` stratum. Count
instrument, register, density, and exposure variants inside those strata, and
do not treat copies of one source progression as independent chord evidence.
The 70% melody gate should be audited after realization; its aggregate
probability is not a substitute for per-stratum balance.

Additional generation is justified when it supplies new chord progressions,
voicings, event timings, accompaniment instruments, and independently sampled
melody conditions for under-covered strata. It is not justified merely because
the global event counter is below a larger target.

Use a melody-only classifier/probe and the cross-condition ACR tests as
stopping checks. If melody-only features predict labels well, or if
accompaniment-identical examples change substantially across melody
conditions, increase randomization and balance before increasing corpus size.
...
previous melody pitch, with metric/exposure weights, distance and maximum-leap
...
| Per-timepoint label safety | Symbolic source labels and audio recognizer decisions are different objects; added melody can change the latter without changing the former. | Make `transparent` and `stress` label-safety policies explicit. Enforce source-event/NCT rules for transparent output and measure ACR changes for stress output. | Evidence-backed distinction |
| Audio evaluation | Melody extraction and source separation are distinct inference problems with their own failure modes. | Preserve oracle V2 and accompaniment stems, and report symbolic, full-mix, oracle-stem, and estimated-separation results separately. | Evidence-backed |
...
2. **High-priority musical terms:** active chord-degree compatibility on strong
   positions, explicit DCT/extension condition, and short-horizon resolution
   for approach/passing/neighbor/enclosure candidates.
...
The `transparent` policy is the default symbolic safety envelope:
...
The `stress` policy is an intentional counterexample envelope. It can allow
dense, chromatic, or same-register material that makes an ACR error more
likely, but it cannot modify the source label, source-event duration, or
source identity. This distinction prevents a successful symbolic preservation
check from being misreported as proof of audio-level recognizer stability.
...
horizon. A strong-beat NCT is allowed when its condition and short-horizon
resolution score justify it.
...
1. emit transparent configuration values and effective values in the manifest;
2. report CT/NCT, role, density, duration, interval, range, and resolution
   distributions by genre, profile, and exposure condition;
...
4. keep externally calibrated priors separate from the generated chord labels;
   and
5. retain an accompaniment-only control for every audio comparison.

When no reference corpus is available, acceptance should focus on invariants,
reproducibility, role/provenance correctness, and controlled condition
differences rather than an unsupported claim of human-likeness.
...
tension/resolution, and multiple recorded exposure conditions. A later
sequence decoder can add stronger long-range scoring. The predecessor's
